    ...
    * realiza la modifcacion 
    ...
    public function modificarEstadoCompra($objCompra,$idCET){
        ...
        if($ultimoEstadoCompra==null){ // es la primera compra (solo se da de alta)
            ...
            $nuevosDatos =['idcompra'=>$idCompra,'idcompraestadotipo'=>$idCET,
            'cefechaini'=>$fechafin,'cefechafin'=>null];
            $resp2=$objCompraEstado->alta($nuevosDatos);
...
/** METODO HACER-COMPRA
 * Solo crea una nueva compra, los itmes los guarda en session y los crea al llamar a la funcion finalizarCompra
 * 
 * @param array productosUsuario (array asociativo con claves : idusuario - idproducto)
 * @return object  // devuelve el objCompra
 */
public function iniciarCompra($productosUsuario){
